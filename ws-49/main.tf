...
resource "kubernetes_config_map_v1" "legacy" {
  metadata {
    name      = "legacy-config"
    namespace = "dadgarcorp-ws49"
  }
  data = {
    note = "already migrated -- do not touch"
  }
}
...
import {
  ...
  id = "dadgarcorp-ws49/app-config"
}

import {
  to = kubernetes_service_account_v1.sa
  id = "dadgarcorp-ws49/deployer"
}
